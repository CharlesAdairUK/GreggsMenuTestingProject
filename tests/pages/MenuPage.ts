  ...
  async getCategoryOptions(): Promise<string[]> {
    ...
    const categoryOptions = await this.page.$$eval(
      ...
      (elements) =>
        elements.map(
          (el) =>
            el.getAttribute('data-testid')?.replace('category-', '') || '',
        ),
    )
...
export class MenuItemElement {
  ...
  constructor(element: Locator, page: Page) {
    ...
    this.name = element
      ...
      .first()
    this.price = element.locator('.price, [data-testid="price"], .cost').first()
    ...
    this.addToBasketButton = element
      ...
      .first()
    this.allergenBadges = element.locator('.allergen, [data-testid="allergen"]')
